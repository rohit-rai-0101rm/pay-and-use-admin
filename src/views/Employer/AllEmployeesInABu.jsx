...
import NoLoanProductWeeklyAndBiweekly from 'components/Errors/NoLoanProductWeeklyAndBiweekly'
import NoLoanProductMonthly from 'components/Errors/NoLoanProductMonthly'

const ref = React.createRef();



const AllEmployeesInABu = () => {
  const { loadiing, employeesInBu, success } = useSelector(state => state.allEmployeesInBu)
const {salaryExist,salaryCycle,loanProductExist}=useSelector((state)=>state.salaryScheduleExists.salaryScheduleExists)
  ...
  function openModal() {
    dispatch(getEmployeeAuthority(buCode))
    console.log("hii")
    setIsOpen(true);
  }
  ...
  useEffect(() => {
    ...
    dispatch(employeesInBuApi(buCode))
  }, [dispatch, buCode, loadiing,loanProductExist])
  const color = "light"
  ...
  if(!loanProductExist){
    if(salaryCycle==="Monthly"){
      <NoLoanProductMonthly buCode={buCode}/>
  
    }
    else{
      <NoLoanProductWeeklyAndBiweekly buCode={buCode}/>
    }
  }
  if (success && employeesInBu.length === 0) {
    return (
      <NoEmployeesInBu code={buCode} />
    )
  }
  ...
  console.log("salary",salaryExist,salaryCycle,loanProductExist)
  return (
...
export default AllEmployeesInABu