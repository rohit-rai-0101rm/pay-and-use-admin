...
const FilteredEmployees = ({ employeeInMultipleLeandBu }) => {
  ...
  console.log(employeeInMultipleLeandBu)
  ...
  const COLUMNS = [
...

    {
      ...
      width: "12.5%",
      Cell: ({ row }) => {
        return <button onClick={() => handleViewAction(row.original.empCode, row.original.buCode)} class="bg-whiteSmoke-500 text-black active:bg-lightBlue-500 font-bold uppercase text-xs px-3 py-2 rounded shadow hover:shadow-md outline-none focus:outline-none  mb-1 ease-linear transition-all duration-150">
          view
        </button>;
      },
    },
  ...
  const data = employeeInMultipleLeandBu
  console.log(data)
  const {
    getTableProps,
    getTableBodyProps,
