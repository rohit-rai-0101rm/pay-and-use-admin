...
import BiweeklySalarySchedule from './BiweeklySalarySchedule'
...
let salaryScheduleFiltered=null
  console.log("salarySchedule", salarySchedule)
  salaryScheduleFiltered = salarySchedule?.filter(item => (moment().isBefore(moment(item?.startDate).format("DD-MM-YYYY"))));
  ...
  const { isMonthly, isWeekly, isBiweekly } = basicInfo
  const [isChecked, setIsChecked] = useState(false)
  console.log(isMonthly, isWeekly, isBiweekly)
  ...
  if (salarySchedule) {
    ...
    for (const element of salarySchedule) {
      ...
      element.salaryType = isMonthly ? "Monthly" : "Weekly"
    }
  }
  const handleSave = () => {
    if (!isChecked) {
  ...
  if (isBiweekly) {
    return <BiweeklySalarySchedule buCode={buCode} biWeekly={isBiweekly} />
  }
