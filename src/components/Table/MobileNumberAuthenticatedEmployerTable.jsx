...
const MobileNumberAuthenticatedEmployerTable = ({ data }) => {
    ...
    const COLUMNS = [
        ...
        {
            Header: "Group Name",
            accessor: "companyGroupDescription",
        ...
        {
            Header: "PAY DAY ELIGIBLE AMOUNT",
            accessor: "payDayAmount",
            Cell: row => <div style={{ textAlign: "right" }}>{row.value == null ? "NA":row.value.toFixed(2)}</div>



        }, {
            Header: "SHORT TERM ELGIBLE AMOUNT",
            accessor: "shortTermAmount",
            Cell: row => <div style={{ textAlign: "right" }}>{row.value == null ? "NA":row.value.toFixed(2)}</div>
    ...
    const handleProceed = (buCode, empCode) => {
                            ...
                            {headerGroups.map(headerGroup => (
                                <tr className="border-t-0 px-6 py-4 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4" {...headerGroup.getHeaderGroupProps()}>
                                    {headerGroup.headers.map(column => (
                                        <th className={
                                            "px-6 font-extrabol  border border-solid py-6 mt-4 text-xs uppercase border-l-0 border-r-0 whitespace-nowrap  text-left " +
                                            (color === "light"
...
export default MobileNumberAuthenticatedEmployerTable